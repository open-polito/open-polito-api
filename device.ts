...
export class Device {
	 ...
	 */
	constructor(uuid: string, timeout = 3000, request_logger: RequestLogger = () => { /* No logging */ }, base_url = "https://app.didattica.polito.it/") {
		this.uuid = uuid;
	...
	async post(endpoint: string, data: { [key: string]: any }): Promise<APIResponse> {
		if (this.token === null)
			throw new Error("No token configured: you must login first");
